...
def mock_session_local():
    with patch("user.infra.repository.user_repo.SessionLocal", autospec = True) as mock:
        yield mock

def test_find_by_email_user_exist(mock_session_local):
...
def test_find_by_email_user_not_exist(mock_session_local):
    ...
    with pytest.raises(HTTPException) as exception:
        user_repository.find_by_email("nonexist@example.com")
